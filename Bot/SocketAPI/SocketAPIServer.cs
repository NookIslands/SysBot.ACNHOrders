    ...
    public sealed class SocketAPIServer
    {
        ...
        private async void HandleTcpClient(TcpClient client)
        {
            ...
            while (true)
            {
                ...
                {
                    Logger.LogError($"[SocketAPI] Exception while reading from client: {ex}\nStackTrace: {ex.StackTrace}");
                    break;
                }

                if (bytesRead == 0)
                {
                    Logger.LogInfo("[SocketAPI] A remote client closed the connection.");
                    break;
                }

                string rawMessage = Encoding.UTF8.GetString(buffer, 0, bytesRead);
                Logger.LogInfo($"[SocketAPI] Received {bytesRead} bytes: {rawMessage}");

                rawMessage = Regex.Replace(rawMessage, @"\r\n?|\n|\0", "");

                SocketAPIRequest? request = null;
                try
                {
                    request = SocketAPIProtocol.DecodeMessage(rawMessage);
                }
                catch (Exception ex)
                {
                    Logger.LogError($"[SocketAPI] Exception during DecodeMessage: {ex}\nStackTrace: {ex.StackTrace}");
                }

                if (request == null)
                {
                    Logger.LogError($"[SocketAPI] Failed to decode message: {rawMessage}");
                    this.SendResponse(client, SocketAPIMessage.FromError("There was an error while JSON-parsing the provided request."));
                    continue;
                }

                Logger.LogInfo($"[SocketAPI] Decoded request: id={request.id}, endpoint={request.endpoint}, args={request.args}");

                SocketAPIMessage? message = null;
                try
                {
                    Logger.LogInfo($"[SocketAPI] Invoking endpoint: {request.endpoint} with args: {request.args}");
                    message = this.InvokeEndpoint(request.endpoint!, request.args);
                }
                catch (Exception ex)
                {
                    Logger.LogError($"[SocketAPI] Exception during endpoint invocation: {ex}\nStackTrace: {ex.StackTrace}");
                }

                if (message == null)
                {
                    Logger.LogError($"[SocketAPI] Endpoint '{request.endpoint}' not found or returned null.");
                    message = SocketAPIMessage.FromError("The supplied endpoint was not found.");
                }
                else
                {
                    Logger.LogInfo($"[SocketAPI] Endpoint '{request.endpoint}' returned: {SocketAPIProtocol.EncodeMessage(message)}");
                }
            ...
            Logger.LogInfo("[SocketAPI] Exiting client handler loop.");
        }
        ...
        {
            string encoded = SocketAPIProtocol.EncodeMessage(message)!;
            Logger.LogInfo($"[SocketAPI] Sending to client: {encoded}");
            byte[] wBuff = Encoding.UTF8.GetBytes(encoded);
            try
            {
            ...
            {
                Logger.LogError($"[SocketAPI] There was an error while sending a message to a client: {ex.Message}\n{ex}");
                toClient.Close();
            }
        }
        ...
        public void Stop()
        {
            Logger.LogInfo("[SocketAPI] Server stop requested.");
